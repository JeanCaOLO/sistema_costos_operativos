...
import { formatCellValue, getColumnTotal, COLUMN_TYPES } from '@/types/mano_obra';
import ManoObraTableRow from './ManoObraTableRow';
...
interface ManoObraTableProps {
  ...
  isAdmin: boolean;
  /** filaId -> colId -> valor desencriptado */
  decryptedData: Record<string, Record<string, string>>;
  onAddColumn: () => void;
...
export default function ManoObraTable({
  columnas, filas, savingId, isAdmin, decryptedData,
  onAddColumn, onEditColumn, onDeleteColumn,
  onAddFila, onUpdateFila, onUpdateCell, onDeleteFila,
}: ManoObraTableProps) {
  // Non-admin: filter out sensitive columns entirely
  const visibleColumnas = isAdmin ? columnas : columnas.filter(c => !c.is_sensitive);
  const hasData = filas.length > 0;

  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      {/* Security banner for admin */}
      {isAdmin && columnas.some(c => c.is_sensitive) && (
        <div className="flex items-center gap-2 px-4 py-2.5 bg-amber-50 border-b border-amber-200">
          ...
          <p className="text-xs text-amber-700">
            <strong>Modo Admin:</strong> Estás visualizando campos sensibles desencriptados. Esta información es confidencial.
          </p>
        </div>
      )}

      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-sm" style={{ minWidth: `${460 + visibleColumnas.length * 160}px` }}>
          <thead>
            <tr className="bg-slate-800">
              <th className="sticky left-0 z-20 bg-slate-800 px-4 py-3.5 text-left w-40 min-w-[160px] border-r border-slate-700">
                <span className="text-xs font-semibold text-slate-200 uppercase tracking-wider">Nombre</span>
              </th>
              <th className="px-4 py-3.5 text-left w-36 min-w-[140px] border-r border-slate-700">
                <span className="text-xs font-semibold text-slate-200 uppercase tracking-wider">Área</span>
              </th>
              <th className="px-4 py-3.5 text-left w-32 min-w-[120px] border-r border-slate-700">
                <span className="text-xs font-semibold text-slate-200 uppercase tracking-wider">Tipo</span>
              </th>
                ...
                <th key={col.id} className="px-4 py-3.5 text-left min-w-[140px] border-r border-slate-700 group/colhead">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5 min-w-0">
                      ...
                      {col.is_sensitive && (
                        <div className="w-3 h-3 flex items-center justify-center text-amber-400 shrink-0" title="Dato sensible encriptado">
                    ...
                    <div className="flex items-center gap-0.5 opacity-0 group-hover/colhead:opacity-100 transition-opacity flex-shrink-0">
                      <button onClick={() => onEditColumn(col)} className="w-5 h-5 flex items-center justify-center rounded text-slate-400 hover:text-white hover:bg-slate-600 transition-colors cursor-pointer" title="Editar">
                        <i className="ri-pencil-line text-xs" />
                      </button>
                      <button onClick={() => onDeleteColumn(col.id)} className="w-5 h-5 flex items-center justify-center rounded text-slate-400 hover:text-rose-400 hover:bg-slate-600 transition-colors cursor-pointer" title="Eliminar">
              ...
              <th className="px-3 py-3.5 w-12 min-w-[48px]">
                <button onClick={onAddColumn} className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-700 hover:bg-emerald-600 text-slate-300 hover:text-white transition-all cursor-pointer" title="Agregar columna">
            ...
            {hasData ? (
              filas.map(fila => (
                <ManoObraTableRow
                  key={fila.id}
                  fila={fila}
                  columnas={visibleColumnas}
                  isAdmin={isAdmin}
                  decryptedMap={decryptedData[fila.id] ?? {}}
              ...
              <tr>
                <td colSpan={visibleColumnas.length + 4} className="px-8 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      <p className="text-slate-600 font-medium text-sm">Sin registros de mano de obra</p>
          ...
          {hasData && visibleColumnas.length > 0 && (
            <tfoot>
              <tr className="bg-slate-50 border-t-2 border-slate-200">
                ...
                <td className="px-4 py-3 border-r border-slate-200" />
                {visibleColumnas.map(col => {
                  // Sensitive totals only for admin, using decrypted values
                  if (col.is_sensitive) {
                    if (!isAdmin) {
                      return <td key={col.id} className="px-4 py-3 border-r border-slate-200"><span className="text-xs text-slate-300">—</span></td>;
                    }
                    const nums = filas.map(f => {
                      const dec = decryptedData[f.id]?.[col.id];
                      return Number(dec ?? 0);
                    }).filter(n => !isNaN(n) && n !== 0);
                    const total = nums.length > 0 ? nums.reduce((a, b) => a + b, 0) : null;
                    return (
                      <td key={col.id} className="px-4 py-3 border-r border-slate-200">
                        {total !== null ? (
                          <span className="text-sm font-bold text-amber-700">{formatCellValue(total, col.tipo)}</span>
